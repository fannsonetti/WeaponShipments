...
{
    public static class GarageLoader
    {
        ...
        public static void LoadGarageAdditiveOnce()
        {
            ...
            using (var stream = asm.GetManifestResourceStream(GARAGE_RESOURCE_NAME))
            {
                if (stream == null)
                {
                    MelonLogger.Error($"[garage] Embedded resource not found: {GARAGE_RESOURCE_NAME}");
                    MelonLogger.Msg("[garage] Available resources:");
            ...
            if (bundle == null)
            {
                MelonLogger.Error("[garage] AssetBundle.LoadFromMemory failed.");
                return;
            }
            ...
            try
            {
                ...
                {
                    MelonLogger.Error("[garage] Bundle contains no prefabs.");
                    return;
                }
                ...
                // --------------------------------------------------
                inst.transform.position = new Vector3(-57.655f, -9.1158f, 153.08f);
                inst.transform.rotation = Quaternion.Euler(270f, 90f, 0f);
                inst.transform.localScale = new Vector3(270f, 260f, 60f);
                ...
                MelonLogger.Msg(
                    $"[garage] Spawned in scene '{activeScene.name}' at {inst.transform.position}"
                );
            }
            finally
            {
        ...
        private static void ForceRuntimeShaderAndRebind(GameObject root)
        {
            ...
            if (fallback == null)
            {
                MelonLogger.Error("[garage] No runtime shader available.");
                return;
            }
